...
def model_setup(mp, k, bc, od, c, reaction_rates, diagenetic_reactions):
    ...
    for species, v in bc.items():
        ...
        scalar = getattr(sc, species)
        f = getattr(reaction_rates, species) * v[5]

        equation_dict[species] = (D * scalar.d("z", 2) - od.w * scalar.d("z", 1), f)
    ...
    for i in range(mp.max_loops):
        ...
        for var_name in bc:  # relax reaction estimates
            n = getattr(reaction_rates, var_name)  # get new value for var name
            o = getattr(p, var_name)  # get old value for var name
            # make sure nan values don't interfere
            v = o * (1 - mp.relax) + mp.relax * n  # relax
        ...
        for var_name in vars(c):  # relax concentrations
            n = getattr(c, var_name)
            v = relax_solution(results[var_name], n, mp.relax)
